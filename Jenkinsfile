pipeline {
	...
	stage('Clean Existing Images and Stop and remove Running container') {            
			...
			sh "docker image rm -f rakesh1533/samplewebapp"
		    }
            }
        }
        stage('Docker Build and Tag') {
           steps {
                ...
                sh 'docker tag samplewebapp rakesh1533/samplewebapp:latest'       
          }
        }
      ...
      stage('Run Docker container') {
            ...
            steps {
                sh "docker run --name samplewebapp -d -p 9090:8080 rakesh1533/samplewebapp"
            }
        }
    }
}
